...
def predict_dist(model: Module, data_fp: str, data_verif_fp: str, output_dir: str, x_filter: list, y_filter: list):
    ...
    plt.clf()
    fig, ax = plt.subplots()
    ax.plot(df_train['rgeo'], df_train['rgeo_NN'], 'bx', alpha=0.1, ms=2, label='Train')
    ax.plot(df_verif['rgeo'], df_verif['rgeo_NN'], 'gx', alpha=0.1, ms=2, label='Test')
    ...
    plt.clf()
    fig, ax = plt.subplots()
    ax.loglog(df_train['rgeo'], df_train['rgeo_NN'], 'bx', alpha=0.1, ms=2, label='Train')
    ax.loglog(df_verif['rgeo'], df_verif['rgeo_NN'], 'gx', alpha=0.1, ms=2, label='Test')
    ax.axline((0, 0), (1, 1), color='k')
    ax.set(xlabel='True [Parsec]', ylabel='Predicted [Parsec]')
    plt.legend()
    fig.tight_layout()
